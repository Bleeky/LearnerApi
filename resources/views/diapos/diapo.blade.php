    ...
    <div class="container">
        ...
        @if (!empty($diapos))
            <a style="text-decoration: none;" class="btn btn-success"
               href="{{ URL::action('DiapoAdminController@getInsertDiapo', [0, $diapos[0]['module_id']]) }}">Insérer une diapo</a>
            ...
            @if (!empty($elem)  && !empty($elem['id']))
                <a style="text-decoration: none; margin-top: 13px; margin-bottom: 13px;" class="btn btn-success"
                   href="{{ URL::action('DiapoAdminController@getInsertDiapo', [$elem['id'], $diapos[0]['module_id']]) }}">Insérer une diapo</a>
                    @endif
            @endforeach
        @else
