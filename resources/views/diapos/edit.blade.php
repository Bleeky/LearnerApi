@extends('layouts.default')

@section('content')
    @include('diapos.form.selector')
    @if ($diapo['content'][0]->type == "1")
        @include('diapos.form.form1')
    @elseif($diapo['content'][0]->type == "2")
        @include('diapos.form.form2')
    @elseif($diapo['content'][0]->type == "3")
        @include('diapos.form.form3')
    @elseif($diapo['content'][0]->type == "4")
        @include('diapos.form.form4')
    @elseif($diapo['content'][0]->type == "5")
        @include('diapos.form.form5')
    @elseif($diapo['content'][0]->type == "6")
        @include('diapos.form.formquestion1')
    @elseif($diapo['content'][0]->type == "7")
        @include('diapos.form.formquestion3')
    @elseif($diapo['content'][0]->type == "8")
        @include('diapos.form.formquestion2')
    @elseif($diapo['content'][0]->type == "9")
        @include('diapos.form.formvideo')
    @endif
@stop